...
use metadata::backups::Backup;
use metadata::backups::CreateBackupRequest;
use metadata::backups::Provider;
...
pub async fn init(md: Arc<MetadataProvider>, db: Arc<OptiDBImpl>, cfg: Config) -> Result<()> {
    ...
    for backup in backups {
        if matches!(backup.status, backups::Status::InProgress(_)) {
            let p = cfg
                .data
                .path
                .join(DATA_PATH_BACKUP_TMP)
                .join(backup.id.to_string());
            if p.exists() {
                fs::remove_file(p)?;
            }
            md.backups.update_status(
                backup.id,
                backups::Status::Failed("server restarted".to_string()),
            )?;
        };
    }
    ...
    tokio::spawn(async move {
        loop {
            ...
            let maybe_idle = backups
                .into_iter()
                .find(|b| b.status == backups::Status::Idle);
            if let Some(backup) = maybe_idle {
                ...
                .await
                {
                    Ok(b) => {
                        md_cloned
                            .backups
                            .update_status(b.id, metadata::backups::Status::Completed)
                            .expect("update status error");
                    }
                    Err(e) => {
                        ...
                        md_cloned
                            ...
                            .update_status(
                                backup.id,
                                metadata::backups::Status::Failed(e.to_string()),
                            )
                            .expect("update status error");
                    }
                };
            ...
            let cron = match settings.backup_schedule_interval {
                ...
                BackupScheduleInterval::Daily => {
                    format!("0 {} * * *", settings.backup_schedule_start_hour)
                }
                BackupScheduleInterval::Weekly => {
                    format!("0 {} * * 0", settings.backup_schedule_start_hour)
                }
                BackupScheduleInterval::Monthly => {
                    format!("0 {} 1 * *", settings.backup_schedule_start_hour)
                }
                BackupScheduleInterval::Yearly => {
                    format!("0 {} 1 1 *", settings.backup_schedule_start_hour)
                }
            };
            ...
            let next_time = cron
                .find_next_occurrence(
                    &DateTime::from_timestamp(cur_time.and_utc().timestamp(), 0).unwrap(),
                    true,
                )
                .expect("find next occurrence error")
                .naive_utc();
            ...
            .await
            {
                ...
                Err(e) => {
                    ...
                    md_cloned
                        .backups
                        .update_status(backup.id, metadata::backups::Status::Failed(e.to_string()))
                        .expect("update status error");
                }
            };
        }
...
async fn run_backup(
    ...
    let hnd = task::spawn_blocking(move || {
        let progress = |pct: usize| {
            md_cloned
                .backups
                .update_status(backup_cloned.id, metadata::backups::Status::InProgress(pct))
                .expect("update status error");
        };
    ...
    if matches!(backup.provider, Provider::Local(_)) {
        ...
        md.backups
            .update_status(backup.id, backups::Status::Uploading)?;
...
fn backup_to_tmp<F: Fn(usize)>(
    ...
    let options = if backup.is_encrypted {
        options.with_aes_encryption(
            AesMode::Aes128,
            backup.password.as_ref().expect("password not set"),
        )
    } else {
        options
    };
...
fn backup_local(backup: &Backup, cfg: &Config, tmp_path: &PathBuf) -> Result<()> {
    let dst_path = cfg.data.path.join(DATA_PATH_BACKUPS).join(
        backup
            .created_at
            .format("%Y-%m-%dT%H:00:00.zip")
            .to_string(),
    );
