    ...
    for row in data.lines().skip(3) {
        ...
        for ((idx, val), field) in v.into_iter().enumerate().zip(fields.iter()) {
            match field.data_type() {
                ...
                | DataType::Utf8
                | DataType::Decimal(_, _)
                | DataType::Timestamp(_, _) => out[idx].push(Value::parse(val, field.data_type())?),
    ...
    let result = out
        ...
        .map(|(vals, field)| match field.data_type() {
            ...
            DataType::Decimal(_, _) => {
                ...
                let a = integer_to_decimal(&a, DECIMAL_PRECISION as usize, DECIMAL_SCALE as usize);
                a.boxed()
            }
    ...
    let fields = fields[0..fields.len() - 1]
        ...
        .map(|v| {
            ...
            let dt = match ty {
                ...
                "decimal" => DataType::Decimal(DECIMAL_PRECISION as usize, DECIMAL_SCALE as usize),
        ...
        match exclusive_row_groups_periodicity {
            ...
            Some(n) if cur_row_group % n == 0 => {
                ...
                let out = chunk
                    ...
                    .iter()
                    .map(|arr| {
                        let take_idx = PrimitiveArray::from_vec((idx as i64..end as i64).collect());
...
"#;
        ...
        let res = crate::test_util::parse_markdown_table_v1(data).unwrap();
