...
use crate::segment::expressions::single_row::expr::Expr;
...
impl<Op> Expr for ValueOp<Option<&str>, Op> where for<'a> Op: BooleanOp<Option<&'a str>> {
    fn evaluate(&self, batch: &RecordBatch, row_id: usize) -> DatafusionResult<bool> {
        ...
        return if col.is_null(row_id) {
            ...
            Ok(Op::perform(Some(col.as_any().downcast_ref::<StringArray>().unwrap().value(row_id)), self.right.clone()))
        };
    }
}

impl<Op> Expr for ValueOp<Option<i8>, Op> where Op: BooleanOp<Option<i8>> {
    fn evaluate(&self, batch: &RecordBatch, row_id: usize) -> DatafusionResult<bool> {
        let col = batch.columns()[self.left_col_id].as_ref();
        return if col.is_null(row_id) {
            Ok(Op::perform(None, self.right))
        } else {
            Ok(Op::perform(Some(col.as_any().downcast_ref::<Int8Array>().unwrap().value(row_id)), self.right))
        };
    }
}

impl<Op> Expr for ValueOp<Option<i64>, Op> where Op: BooleanOp<Option<i64>> {
    fn evaluate(&self, batch: &RecordBatch, row_id: usize) -> DatafusionResult<bool> {
        let col = batch.columns()[self.left_col_id].as_ref();
        return if col.is_null(row_id) {
            Ok(Op::perform(None, self.right))
...
impl<Op> Expr for ValueOp<Option<bool>, Op> where Op: BooleanOp<Option<bool>> {
    fn evaluate(&self, batch: &RecordBatch, row_id: usize) -> DatafusionResult<bool> {
        let col = batch.columns()[self.left_col_id].as_ref();
        return if col.is_null(row_id) {
            Ok(Op::perform(None, self.right))
        } else {
            Ok(Op::perform(Some(col.as_any().downcast_ref::<BooleanArray>().unwrap().value(row_id)), self.right))
        };
    }
}

impl<Op> Expr for ValueOp<i8, Op> where Op: BooleanOp<i8> {
    fn evaluate(&self, batch: &RecordBatch, row_id: usize) -> DatafusionResult<bool> {
        let col = batch.columns()[self.left_col_id].as_ref();
        Ok(Op::perform(col.as_any().downcast_ref::<Int8Array>().unwrap().value(row_id), self.right))
    }
}
...
mod tests {
    ...
    use arrow::record_batch::RecordBatch;
    use crate::segment::expressions::boolean_op::{Eq, Gt};
