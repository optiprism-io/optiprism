...
impl Sequence {
    pub fn new(steps: Vec<Arc<dyn PhysicalExpr>>, exclude: Option<Vec<(Arc<dyn PhysicalExpr>, Vec<usize>)>>) -> Self {
        Self {
            steps,
            exclude,
        }
...
impl Expr<bool> for Sequence {
    fn evaluate(&self, batch: &RecordBatch, _: usize) -> bool {
        ...
        while row_id < batch.num_rows() {
            // first check if event is a step
            if steps[step_id].value(row_id) {
                // println!("plus. row_id: {} step_id: {}", row_id, step_id);
                step_id += 1;
            ...
            for i in 0..exclude[step_id].len() {
                if exclude[step_id][i].value(row_id) {
                    // println!("minus. row_id: {} step_id: {}", row_id, step_id);
                    if step_id > 0 {
                        step_id -= 1;
                    }
...
mod tests {
    ...
    fn test() -> Result<()> {
        ...
        let op = Sequence::new(vec![Arc::new(step1), Arc::new(step2), Arc::new(step3)], None);
    ...
    fn test_exclude() -> Result<()> {
        ...
        });
        {
            ...
            let op = Sequence::new(vec![step1.clone(), step2.clone(), step3.clone()], exclude);
            assert_eq!(true, op.evaluate(&batch, 0));
        }
