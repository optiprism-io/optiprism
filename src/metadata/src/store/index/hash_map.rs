...
impl HashMap {
    ...
    pub fn check_insert_constraints(&self, keys: &[Option<Vec<u8>>]) -> Result<()> {
    ...
    pub fn insert<V: AsRef<[u8]>>(&self, keys: &[Option<Vec<u8>>], value: V) -> Result<()> {
        for key in keys.iter().flatten() {
            self.store.put(key, value.as_ref())?;
        }
        Ok(())
    }
    ...
    pub fn check_update_constraints(
        ...
        for (key, prev_key) in keys.iter().zip(prev_keys) {
            if let Some(key_v) = key {
                if key != prev_key && (self.store.get(key_v)?).is_some() {
                    return Err(
    ...
    pub fn delete(&self, keys: &[Option<Vec<u8>>]) -> Result<()> {
        for key in keys.iter().flatten() {
            self.store.delete(key)?;
        }
        Ok(())
