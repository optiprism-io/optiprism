...
fn test_properties() -> Result<()> {
    ...
    let res = md.event_properties.create(1, create_prop3.clone())?;
    assert_eq!(res.id, 3);
    assert_eq!(res.order, 0);
    // check existence by id
    assert_eq!(md.event_properties.get_by_id(1, 1)?.id, 1);
    assert_eq!(md.event_properties.get_by_id(1, 2)?.id, 2);
    // by name

    assert_eq!(md.event_properties.get_by_name(1, "prop1")?.id, 1);
    assert_eq!(md.event_properties.get_by_name(1, "prop2")?.id, 2);
    let mut update_prop1 = update_prop_req.clone();
    update_prop1.name.insert("prop2".to_string());
    let _a = md.event_properties.update(1, 1, update_prop1.clone());
    assert_eq!(
        md.event_properties.update(1, 1, update_prop1.clone())?.name,
        "prop1".to_string()
    );
    update_prop1.name.insert("prop1_new".to_string());
    let res = md.event_properties.update(1, 1, update_prop1.clone())?;
    assert_eq!(res.id, 1);

    assert!(md.event_properties.get_by_name(1, "prop1").is_ok());

    update_prop1.display_name.insert(Some("e".to_string()));
    assert_eq!(
        md.event_properties
            .update(1, 1, update_prop1.clone())?
            .display_name,
        Some("e".to_string())
    );
    ...
    assert_eq!(md.event_properties.list(1)?.data[0].id, 1);

    // delete props
    assert_eq!(md.event_properties.delete(1, 1)?.id, 1);
    assert!(md.event_properties.get_by_id(1, 1).is_err());
    assert!(md.event_properties.get_by_name(1, "prop1_new").is_err());
    assert_eq!(md.event_properties.delete(1, 2)?.id, 2);
    assert!(md.event_properties.get_by_id(1, 2).is_err());
