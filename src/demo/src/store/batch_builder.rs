...
impl RecordBatchBuilder {
    ...
    pub fn build_record_batch(&mut self) -> Result<RecordBatch> {
        let cols: Vec<ArrayRef> = vec![
            ...
            Arc::new(self.product_price.finish().with_precision_and_scale(DECIMAL_PRECISION, DECIMAL_SCALE)?),
            Arc::new(self.product_discount_price.finish().with_precision_and_scale(DECIMAL_PRECISION, DECIMAL_SCALE)?),
            Arc::new(self.spent_total.finish().with_precision_and_scale(DECIMAL_PRECISION, DECIMAL_SCALE)?),
            Arc::new(self.products_bought.finish()),
            Arc::new(self.cart_items_number.finish()),
            Arc::new(self.cart_amount.finish().with_precision_and_scale(DECIMAL_PRECISION, DECIMAL_SCALE)?),
            Arc::new(self.revenue.finish().with_precision_and_scale(DECIMAL_PRECISION, DECIMAL_SCALE)?),
