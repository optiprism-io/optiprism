...
#[derive(Debug)]
pub struct Aggregate<T, Arr, Op, Acc>
where T: Debug
{
...
impl<T, Arr, Op, Acc> Aggregate<T, Arr, Op, Acc>
where
    Acc: Accumulator<T>,
    T: Debug + Default,
    Op: Debug,
    ...
    pub fn try_new(
            ...
            .equals_datatype(&DataType::Boolean)
        {
            return Err(QueryError::Plan(
                "Predicate should have boolean type".to_string(),
            ));
        }
...
impl<T, Arr, Op, Acc> Display for Aggregate<T, Arr, Op, Acc>
where
    T: Debug,
    Op: BooleanOp<T>,
    Acc: Accumulator<T>,
...
macro_rules! gen_evaluate_int {
    ($acc_ty:ty,$array_type:ident) => {
        impl<Op, Acc> Expr for Aggregate<$acc_ty, $array_type, Op, Acc>
        where
            Op: BooleanOp<$acc_ty>,
            Acc: Accumulator<$acc_ty> + Debug,
        {
            fn evaluate(
                &mut self,
                spans: &[usize],
                ...
                let predicate_arr = predicate_arr
                    .as_any()
                        ...
                        match row {
                            ...
                            RowResult::NextRow(row_id) => {
                                if predicate_arr.value(row_id as usize) {
                                    self.left = Acc::perform(
                                        self.left,
                                        left_arr.value(row_id as usize) as $acc_ty,
                                    );
                                }
                                println!("left {}", self.left);
                            }
...
mod tests {
    ...
    fn sum() -> anyhow::Result<()> {
        ...
        let mut op = Aggregate::<i64, Int8Array, BooleanGt, Sum>::try_new(
            &schema,
            valcol.clone(),
            bcol.clone(),
            1,
        ...
        let mut op =
            Aggregate::<i64, Int8Array, BooleanGt, Sum>::try_new(&schema, valcol, bcol.clone(), 2)?;
