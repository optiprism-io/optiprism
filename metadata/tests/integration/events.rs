...
async fn test_events() -> Result<()> {
    ...
    assert!(md.events.get_by_id(1, 1).await.is_err());
    assert!(md.events.get_by_name(1, "test").await.is_err());
    assert!(md.events.delete(1, 1).await.is_err());
    assert!(md.events.update(update_event_req.clone()).await.is_err());
    // assert_eq!(md.events.list_events().await?, vec![]);
    // create two events
    let mut create_event1 = create_event_req.clone();
    create_event1.name = "event1".to_string();
    let res = md.events.create(create_event1.clone()).await?;
    assert_eq!(res.id, 1);
    ...
    // by name
    assert_eq!(md.events.get_by_name(1, "event1").await?.id, 1);
    assert_eq!(md.events.get_by_name(1, "event2").await?.id, 2);
    ...
    // delete events
    assert_eq!(md.events.delete(1, 1).await?.id, 1);
    assert_eq!(md.events.delete(1, 2).await?.id, 2);

    // events should gone now
    assert!(md.events.get_by_id(1, 1).await.is_err());
    assert!(md.events.get_by_id(1, 2).await.is_err());
    assert!(md.events.get_by_name(1, "event1_new").await.is_err());
    Ok(())
}
