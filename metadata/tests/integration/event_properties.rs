...
async fn test_properties() -> Result<()> {
    ...
    assert!(md.event_properties.get_by_name(1, "test").await.is_err());
    assert!(md.event_properties.delete(1, 1).await.is_err());
    assert!(md
        .event_properties
        .update(update_prop_req.clone())
        .await
        .is_err());
    ...
    // by name
    assert_eq!(md.event_properties.get_by_name(1, "prop1").await?.id, 1);
    assert_eq!(md.event_properties.get_by_name(1, "prop2").await?.id, 2);
    let mut update_event1 = update_prop_req.clone();
    update_event1.id = 1;
    update_event1.name = "prop2".to_string();
    assert!(md
        .event_properties
        .update(update_event1.clone())
        .await
        .is_err());
    update_event1.name = "prop1_new".to_string();
    update_event1.scope = Scope::User;
    let res = md.event_properties.update(update_event1.clone()).await?;
    assert_eq!(res.id, 1);
    assert_eq!(res.scope, Scope::User);

    assert!(md.event_properties.get_by_name(1, "prop1").await.is_err());
    assert_eq!(md.event_properties.get_by_name(1, "prop1_new").await?.id, 1);
