...
impl RocksDBTable {
    ...
    pub async fn load(
        ...
        if let Some(num_partitions) = output_partitions {
            ...
            return RocksDBTable::try_new(schema.clone(), output_partitions);
        }
        RocksDBTable::try_new(schema.clone(), data)
    }
}
...
mod tests {
    ...
    async fn test_with_projection() -> Result<()> {
        ...
        let provider = RocksDBTable::try_new(schema, vec![vec![batch]])?;
    ...
    fn test_schema_validation_incompatible_column() -> Result<()> {
        ...
        match RocksDBTable::try_new(schema2, vec![vec![batch]]) {
    ...
    async fn test_merged_schema() -> Result<()> {
        ...
        let provider =
            RocksDBTable::try_new(Arc::new(merged_schema), vec![vec![batch1, batch2]])?;
