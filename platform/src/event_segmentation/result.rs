use arrow::array::{Array, BooleanArray, DecimalArray, Int8Array, Int16Array, Int32Array, Int64Array, UInt8Array, UInt16Array, UInt32Array, UInt64Array, StringArray, Float16Array, Float32Array, Float64Array};
...
impl TryFrom<query::reports::results::DataTable> for DataTable {
    ...
    fn try_from(value: query::reports::results::DataTable) -> std::result::Result<Self, Self::Error> {
            ...
            .map(|column| {
                let data = match column.data_type() {
                    ...
                    DataType::Decimal(_, s) => {
                        let arr = column.data().as_any().downcast_ref::<DecimalArray>().unwrap();
                        arr
                            .iter()
                            .map(|value| match value {
                                ...
                                Some(v) => {
                                    ...
                                    let d_f = match d.to_f64() {
                                        None => return Err(Error::Internal("can't convert decimal to f64".to_string())),
                                        Some(v) => v
                                    };
                                    let n = match Number::from_f64(d_f) {
                                        None => return Err(Error::Internal("can't make json number from f64".to_string())),
                                        Some(v) => v
                                    };
                                    Ok(Value::Number(n))
                                }
